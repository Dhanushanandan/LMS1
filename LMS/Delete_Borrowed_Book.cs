...
namespace LMS
{
    public partial class Delete_Borrowed_Book : Form
    {
        //invoking the database connecting using a seperate class with creating an object
        SqlConnection conn = new Database().DBConnect();
        public Delete_Borrowed_Book()
        {
            InitializeComponent();
        }

        private void Delete_Borrowed_Book_Load(object sender, EventArgs e)
        {
                ...
                else
                {
                    ...
                    if (dataTable1.Rows.Count > 0)
                    {
                        ...
                        if (dataTable2.Rows.Count > 0)
                        {
                            ...
                            SqlCommand cmd = new SqlCommand("Delete from Borrowed_Book Where User_ID=@User_ID AND Book_ID=@Book_ID AND Book_Borrowed_Date=@Book_Borrowed_Date", conn);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd = new SqlCommand("TRUNCATE TABLE Borrowed_Book", conn);
                    cmd.ExecuteNonQuery();
                    MessageBox.Show("Borrowed Details Delete Completed", "ADD", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ID_textBox.Clear();
                    dateTimePicker1.Value = DateTime.Now;
                    conn.Close();
                }
