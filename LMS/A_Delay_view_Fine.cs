...
namespace LMS
{
    public partial class A_Delay_view_Fine : Form
    {
        //invoking the database connecting using a seperate class with creating an object
        SqlConnection conn = new Database().DBConnect();
        public A_Delay_view_Fine()
        {
            InitializeComponent();
        }

        private void A_Delay_view_Fine_Load(object sender, EventArgs e)
        {
        ...
        private void Search_button_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    conn.Open();
                    SqlCommand cmd1 = new SqlCommand("Select * from Fine_Payment Where User_ID=@User_ID OR Fine__ID=@Fine__ID", conn);
                    cmd1.Parameters.AddWithValue("@Fine__ID", Search_textBox.Text);
