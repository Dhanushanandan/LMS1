...
namespace LMS
{
    public partial class U_Borrowed_Book : Form
    {
        //invoking the database connecting using a seperate class with creating an object
        SqlConnection conn = new Database().DBConnect();
        public U_Borrowed_Book()
        {
            InitializeComponent();
        }

        private void U_Borrowed_Book_Load(object sender, EventArgs e)
        {
                ...
                else
                {
                    ...
                    if (dataTable1.Rows.Count > 0)
                    {
                        ...
                        if (dataTable2.Rows.Count > 0)
                        {
                            ...
                            SqlCommand cmd = new SqlCommand("Update Borrowed_Book set Book_Name=@Book_Name Where User_ID=@User_ID AND Book_ID=@Book_ID AND Book_Borrowed_Date=@Book_Borrowed_Date", conn);
